...
private:
    ...
    ros::Publisher wayPointsPub_1;//用于发布另外一条轨迹进行对比
    ros::Publisher trajectoryPub;
    ros::Publisher trajectoryPub_1;//用于发布另外一条轨迹进行对比
...
public:
        ...
        : nh(nh_)
    {
        ...
        trajectoryPub_1 = nh.advertise<visualization_msgs::Marker>("/visualizer/trajectory_1",10);
        wayPointsPub_1 = nh.advertise<visualization_msgs::Marker>("/visualizer/waypoints_1",10);
    }
    ...
    // Visualize the trajectory only
    template <int D>
    inline void visualizeTraj(const Trajectory<D> &traj,
                               const std::string &traj_name)
    {
        visualization_msgs::Marker trajMarker, wayPointMarker;
        wayPointMarker.id = -1;
        wayPointMarker.type = visualization_msgs::Marker::SPHERE;
        wayPointMarker.header.stamp = ros::Time::now();
        wayPointMarker.header.frame_id = "world";
        wayPointMarker.pose.orientation.w = 1.00;
        wayPointMarker.action = visualization_msgs::Marker::ADD;
        wayPointMarker.ns = "waypoints_1";
        wayPointMarker.color.a = 0.5;
        wayPointMarker.color.r = 0.00;
        wayPointMarker.color.g = 1.00;
        wayPointMarker.color.b = 1.00;
        wayPointMarker.scale.x = 0.35;
        wayPointMarker.scale.y = 0.35;
        wayPointMarker.scale.z = 0.35;

        trajMarker = wayPointMarker;
        trajMarker.type = visualization_msgs::Marker::LINE_LIST;
        trajMarker.ns = "trajectory_1";
        trajMarker.color.r = 0.00;
        ...
        if (traj.getPieceNum() > 0){
            ...
            wayPointsPub_1.publish(wayPointMarker);
        }
        ...
        if (traj.getPieceNum() > 0)
        {
            ...
            trajectoryPub_1.publish(trajMarker);
        }
