...
public:
    ...
    inline void mapCallBack(const sensor_msgs::PointCloud2::ConstPtr &msg)
    {
        if (!mapInitialized)
        {
            ...
            //question_1:这里的“膨胀”对地图进行了什么操作,获取到膨胀之后所有障碍物表面点的坐标
    ...
    inline void plan()
    {
        if (startGoal.size() == 2)
        {
            ...
            //question_2:pc代表障碍物表面点
            voxelMap.getSurf(pc);

            //question_3:凸包生成是最大体积限制，在比较空旷的环境中无解是因为凸包生成失败？
            //实验室常用的凸包生成方法
            ...
            if (route.size() > 1)
            {
                ...
                {
                    std::cout <<"first traj optimize failed!!!\n";
                }
                ...
                traj.clear();
                if (std::isinf(gcopter.optimize_1(traj, config.relCostTol)))
                {
                    std::cout <<"second traj optimize failed\n";
                }
                if (traj.getPieceNum() > 0){
                    trajStamp = ros::Time::now().toSec();
                    visualizer.visualizeTraj(traj,"traj2");
                }
            }
        }
    }
